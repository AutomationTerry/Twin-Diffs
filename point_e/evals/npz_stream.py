...
#定义了一个名为 NumpyArrayInfo 的数据类（data class），用于存储关于 .npz 文件中数组的信息。
@dataclass
...
#定义了一个名为 NpzStreamer 的类，用于处理 .npz 文件中的数据流。
class NpzStreamer:
...
#名为 _npz_paths_and_length 的函数，用于处理 .npz 文件的路径和截断长度。
...
#NpzArrayReader 抽象基类  实现 read_batch 方法，用于按批次读取数据。
class NpzArrayReader(ABC):
    @abstractmethod
    def read_batch(self, batch_size: int) -> Optional[np.ndarray]:
        pass

#StreamingNpzArrayReader 类：这是一个具体的数据流读取器，继承自 NpzArrayReader 抽象基类。
#read_batch 方法用于按批次读取数据
...
#名为 MemoryNpzArrayReader 的类，用于从内存中读取 .npz 文件中的数据。
...
#定义了一个名为 open_npz_arrays 的上下文管理器（context manager），用于打开 .npz 文件中的数组。（处理多个数组）
@contextmanager
...
#定义了一个名为 CombinedReader 的类，用于组合多个数据流读取器。
...
#这是一个上下文管理器（context manager），用于打开 .npz 文件中指定名称的数组。（处理单个数组）
@contextmanager
