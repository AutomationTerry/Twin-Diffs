...
#名为 ImageCLIP 的类，它是一个预训练的 CLIP 模型的包装器，可以自动处理文本、图像和嵌入的批量输入。
...
#名为 FrozenImageCLIP 的类，它是对 ImageCLIP 类的一个简单包装，并且冻结了模型的参数，以便在推断时不计算梯度。
...
#_image_to_pil 的作用是将图像对象转换为 PIL 图像对象。
